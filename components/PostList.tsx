    ...
    <div>
      <div
            ...
            post.tags.map((tag: string) => (
              <span
                ...
                className={`px-4 py-0.5 rounded-3xl text-sm font-bold ${
                  badges[tag.toLocaleLowerCase()] ?? 'bg-amber-200 text-amber-800'
                }`}>
        ...
        <Link href={`/blog/${post.slug}`}>
          <h1>
            <a className='font-bold font-nunito text-2xl flex-1 hover:border-b-2 hover:border-rose-500 focus:text-rose-400'>{post.title}</a>
          </h1>
        </Link>
