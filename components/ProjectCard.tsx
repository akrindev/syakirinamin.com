...
function Lightbox({ src, alt, onClose }: { src: string; alt: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    gsap.fromTo(imgRef.current, { opacity: 0, scale: 0.8, y: 20 }, { opacity: 1, scale: 1, y: 0, duration: 0.4, ease: "power3.out" });
  ...
  const handleClose = () => {
    ...
    gsap.to(imgRef.current, { opacity: 0, scale: 0.85, y: 10, duration: 0.25, ease: "power2.in", onComplete: onClose });
  };
